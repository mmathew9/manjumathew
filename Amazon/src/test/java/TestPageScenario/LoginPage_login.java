...
public class LoginPage_login extends DriverBase {
	...
	// Negative Testcase
	  ...
	  obj = new Properties(); log.info(getClass());
	  System.out.println(System.getProperty("user.dir")); objfile = new
	  FileInputStream(System.getProperty("user.dir") +
	  "/util/testdata.properties"); obj.load(objfile);
	  
	  username = obj.getProperty("username"); invalid_password =
	  obj.getProperty("invalid_password"); System.out.println(username);
	  System.out.println(invalid_password);
	  log.info("Retreiving Username and Password from property file");
	  
	  //keys.elementClick(By.className("android.widget.ImageView"));
	  
	  
	  keys.clickElement(wait.until(ExpectedConditions.visibilityOf(loginpage.
	  already_cust_sign)));
	  log.info("Clicked already customer signin successfully");
		 ...
		 * 
		 * keys.sendString(wait.until(ExpectedConditions.visibilityOf(loginpage.
		 * mob_email_box)), username); log.info("Typed email id successfully");
		 * 
		 * keys.clickElement(wait.until(ExpectedConditions.visibilityOf(loginpage.cont))
		 * ); log.info("Clicked continue successfully");
		 * 
		 * keys.sendString(wait.until(ExpectedConditions.visibilityOf(loginpage.pwd)),
		 * invalid_password); log.info("Typed Password successfully");
		 * 
		 ...
		 * if(wait.until(ExpectedConditions.visibilityOf(loginpage.incorrect_login_text)
		 * ).isDisplayed()) { boolean status
		 * =loginpage.incorrect_login_text.isDisplayed(); System.out.println(status);
	  ...
	  username = obj.getProperty("username"); password =
	  obj.getProperty("password"); System.out.println(username);
	  ...
	  already_cust_sign)));
		 ...
		 * 
		 * keys.sendString(wait.until(ExpectedConditions.visibilityOf(loginpage.pwd)),
		 * password); log.info("Typed Password successfully");
		 * 
		 * keys.clickElement(wait.until(ExpectedConditions.visibilityOf(loginpage.login)
		 * )); log.info("Clicked login successfully");
		 */
